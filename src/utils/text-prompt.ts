...
- 如果文本中有整单折扣（如 "Total Discount"、"整单优惠"、"会员折扣"等），提取到 totalDiscount 字段
- totalDiscount 必须是**负数**（如 -5.00 表示减免 5 元）
- **区分规则**：
  - 如果折扣紧跟在某个商品后面（相邻行） → 是商品级别的折扣，作为 attachment 处理
  - 如果折扣在文本底部、接近总计行、或明确标注为"整单" → 是整单折扣，提取到 totalDiscount
- 常见的整单折扣关键词："Total Discount"、"整单折扣"、"会员优惠"、"满减"、"优惠券"、"Coupon"、"Member Discount"
...
1. **total（总金额，必需）**：
   - 从文本中找到 "TOTAL"、"总计"、"合计"、"应付" 等标记
   - 提取对应的金额数字
   - 这是小票的最终应付金额
   - **必须提供此字段**

2. **subtotal（小计，可选）**：
   - 如果文本中有 "SUBTOTAL"、"小计"、"税前合计" 等标记，提取此金额
   - 通常是所有商品价格的总和（税前）
   - **如果文本中没有显示，不要计算，直接不提供此字段**

3. **tax（总税额，可选）**：
   - 如果文本中有单独的税额总计行（如 "TAX"、"GST"、"税额合计" 等），提取此金额
   - 这是所有商品税额的总和
   - **如果文本中没有显示，不要计算，直接不提供此字段**
...
3. TPD 折扣项的 price 是**正数 0.5**（不是 -0.5），这是商品级别的折扣
4. Member Discount 是整单折扣，提取到 totalDiscount 字段，必须是**负数 -2.00**
5. 所有附加费用（押金、商品折扣）的 price 都必须是正数
6. subtotal、tax、totalDiscount 只有在文本中明确显示时才提供，不要计算
7. total 是小票上显示的最终应付金额（必需字段）`;
